...
SELECT MAX(sal_emp), MIN(sal_emp), MAX(sal_emp) - MIN(sal_emp) AS Diferencia_Salario FROM empleados;
SELECT nombre_depto, AVG(sal_emp) AS Salario_Promedio FROM empleados, departamentos GROUP BY nombre_depto;
SELECT id_depto, AVG(sal_emp) AS Salario_Promedio FROM empleados GROUP BY id_depto;

SELECT id_depto, COUNT(id_emp) AS num_empleados FROM empleados GROUP BY id_depto HAVING COUNT(id_emp) > 3;

SELECT dep.nombre_depto, COUNT(emp.id_emp) AS num_empleados FROM departamentos dep
LEFT JOIN empleados emp ON dep.id_depto = emp.id_depto
GROUP BY dep.id_depto
HAVING COUNT(emp.id_emp) > 3;

SELECT id_depto, COUNT(id_emp) AS num_empleados FROM empleados GROUP BY id_depto HAVING COUNT(id_emp) = 0;

SELECT emp.nombre, dept.nombre_depto, dept.nombre_jefe_depto
FROM empleados emp
INNER JOIN departamentos dept ON emp.id_depto = dept.id_depto;

SELECT emp.nombre, dept.nombre_depto, emp.sal_emp
FROM empleados emp
INNER JOIN departamentos dept ON emp.id_depto = dept.id_depto
WHERE emp.sal_emp >= (SELECT AVG(sal_emp) FROM empleados)
ORDER BY dept.nombre_depto;